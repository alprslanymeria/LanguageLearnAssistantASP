...
// TYPES
import { useWritingAddOrEditCustomEffectProps } from "@/src/components/WritingAddOrEditComponent/prop"
import { WBWL } from "@/src/types/actions"
// ACTIONS
import { GetLanguages } from "@/src/actions/language"
import { GetItemById } from "@/src/actions/crud"


export function useWritingAddOrEditCustomEffect(params : useWritingAddOrEditCustomEffectProps) {
    ...
    useEffect(() => {
        ...
        const GET = async () => {
            ...
            try {
                ...
                const response = await GetItemById({itemId, table:"wbooks"})
                ...
                dispatch({ type: "SET_LANGUAGE_ID", payload: { languageId: (response.data.data as WBWL).writing.languageId }})
                dispatch({ type: "SET_INPUT_ONE", payload: {inputOne: (response.data?.data as WBWL).name}})
