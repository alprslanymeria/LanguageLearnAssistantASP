...
export class CompareLanguageIdQueryHandler implements IQueryHandler<CompareLanguageIdQuery, boolean> {
    ...
    async Handle(request: CompareLanguageIdQuery): Promise<boolean> {
        ...
        if(user.nativeLanguageId === request.request.languageId) {

            this.logger.info(`CompareLanguageIdQueryHandler: User's native language ID matches the provided language ID ${request.request.languageId}.`)
            return true
        }

        this.logger.info(`CompareLanguageIdQueryHandler: User's native language ID does not match the provided language ID ${request.request.languageId}.`)
        return false
